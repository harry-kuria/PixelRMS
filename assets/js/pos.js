...
if (auth == undefined) {
    ...
    $(document).ready(function () {
                ...
                data.forEach(item => {
                    ...
                    let item_info = `<div class="col-lg-2 box ${item.category}"
                            ...
                            <div id="image"><img src="${item.img == "" ? "./assets/images/default.jpg" : img_path + item.img}" id="product_img" alt=""></div> 
                                        ...
                                        <span class="stock">STOCK </span><span class="count">${item.stock == 1 ? item.quantity : ''}</span></div>
        ...
        <table width="100%">
            ...
            let transporter = nodemailer.createTransport({
                service: 'gmail',
                auth: {
                    user: 'pixelinktechnologies@gmail.com',
                    pass: 'tqncjwkheqjgdvbm '
                }
            });

            let mailOptions = {
                from: 'PIXELINK POS <pixelinktechnologies@gmail.com>',
                to: settings.email,
                subject: 'Receipt',
                html: receipt
              };
        ...
        $.fn.editProduct = function (index) {
            ...
            $('#product_market_price').val(allProducts[index].costprice);
        ...
        $.fn.editProducts = function (index) {

            $('#Customers').modal('hide');

            

            $('#userName').val(allCustomers[index].name);
            console.log("Name is "+allCustomers[index].name)
            $('#phoneNumber').val(allCustomers[index].phone);
            $('#emailAddress').val(allProducts[index].email);
        ...
        function loadProductList() {
            ...
            let product_list = '';
            let counter = 0;
            let totalCostPrice = 0; // Initialize total cost price
            let totalSellingPrice = 0; // Initialize total selling price
            ...
            products.forEach((product, index) => {
                ...
                totalCostPrice += parseFloat(product.costprice);
                totalSellingPrice += parseFloat(product.price);
                ...
                product_list += `<tr>
                    ...
                    <td>${settings.symbol}${product.costprice}</td>
                    <td>${product.stock == 1 ? product.quantity : ''}</td>
                ...
                if (counter == allProducts.length) {
                    ...
                    // Add total cost price at the end of the product list
                    $('#product_list').append(`<tr><td colspan="4" style="text-align: center;"><strong>Total Cost Price: ${totalCostPrice.toFixed(2)}</strong></td></tr>`);
                    $('#product_list').append(`<tr><td colspan="4" style="text-align: center;"><strong>Total Selling Price: ${totalSellingPrice.toFixed(2)}</strong></td></tr>`);
        ...
        function loadCustomerList() {
            ...
            <td>${product.phone}</td>
            <td>${product.email}</td>
            <td>${product.KRA_PIN}</td>
            <td class="nobr"><span class="btn-group"><button onClick="$(this).editProducts(${index})" class="btn btn-warning btn-sm"><i class="fa fa-edit"></i></button><button onClick="$(this).deleteProduct(${product._id})" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></button></span></td></tr>`;

                if (counter == allCustomers.length) {

                    $('#customer_list').html(customer_list);

                    customers.forEach(pro => {
                        $("#" + pro._id + "").JsBarcode(pro._id, {
                            width: 2,
                            height: 25,
                            fontSize: 14
    ...
    $('#print_list').click(function () {
        ...
        $('#productList').DataTable().destroy();
        // Reinitialize DataTable with new content
        $('#productList').DataTable({
            "order": [[1, "desc"]],
            "autoWidth": false,
            "info": true,
            ...
            "ordering": true,
            "paging": true,
            "dom": 'Bfrtip', // Include this option to specify the button placement
"buttons": [
'csv', 
'excel', 
{
extend: 'pdfHtml5', // Include the PDF button with HTML5 export
footer: true, // Enable footer in PDF
exportOptions: {
    columns: [2, 3, 4, 5, 6] // Exclude the first column (barcode column) from export
},
customize: function(doc) {
    // Add custom content to the footer of the PDF
    doc.content.push({
        text: [
            { text: 'Total Cost Price: ', bold: true, fontSize:15 }, { text: settings.symbol +" "+ parseFloat(totalCostPrice).toFixed(2), bold: true, fontSize:15 },
            { text: '\nTotal Selling Price: ', bold: true, fontSize:15 }, { text: settings.symbol +" "+ parseFloat(totalCostPrice).toFixed(2), bold: true, fontSize:15 },
            { text: '\nExpected Profit: ', bold: true, fontSize:15 }, { text: settings.symbol +" "+ parseFloat(totalSellingPrice - totalCostPrice).toFixed(2), bold: true, fontSize:15 }
        ],
        alignment: 'center',
        margin: [0, 0, 0, 10] // Adjust the margin to position the text at the bottom
    });
}
}
] // Include the desired buttons in this array
});
    ...
    $.get(api + query, function (transactions) {
        if (transactions.length > 0) {
            ...
            transactions.forEach((trans, index) => {
                ...
                let transactionCostPrice = 0; // Initialize the transaction cost price

                trans.items.forEach(item => {
                    let product = products.find(p => p._id === item.id);
                    if (product) {
                        totalCostPrice += parseFloat(product.costprice);
                        transactionCostPrice += parseFloat(product.costprice); // Accumulate the cost price for each item
                    }
                ...
                transaction_list += `<tr>
                                ...
                                <td>${settings.symbol + transactionCostPrice.toFixed(2)}</td> <!-- Display the transaction cost price -->
                ...
                if (counter == transactions.length) {
                    ...
                    $('#total_costPrice #counter').text(settings.symbol + parseFloat(sales).toFixed(2));
                    ...
                    $('#transactionList').DataTable().destroy(); // Destroy existing DataTable instance

                    $('#transaction_list').html(transaction_list);

                    // Append total sales row to the transaction list table
                    transaction_list += `<tr><td colspan="8" align="right"><strong>Total Sales:</strong></td><td>${settings.symbol + parseFloat(sales).toFixed(2)}</td></tr>`;
                    transaction_list += `<tr><td colspan="10" align="right"><strong>Total Cost Price:</strong></td><td>${settings.symbol + parseFloat(totalCostPrice).toFixed(2)}</td></tr>`;
                        ...
                        "buttons": [
                            ...
                            'excel', 
                            {
                                ...
                                customize: function(doc) {
                                    ...
                                    doc.content.push({
                                        text: [
                                            { text: 'Total Sales: ', bold: true, fontSize:15 }, { text: settings.symbol +" " + parseFloat(sales).toFixed(2), bold: true, fontSize:15 },
                                            { text: '\n Total Cost Price: ', bold: true, fontSize:15 }, { text: settings.symbol +" " + parseFloat(totalCostPrice).toFixed(2), bold: true, fontSize:15 },
                                            { text: '\n Total Profit: ', bold: true, fontSize:15 }, { text: settings.symbol +" " + parseFloat(sales-totalCostPrice).toFixed(2), bold: true, fontSize:15 }
                                        ],
                                        alignment: 'center',
                                        margin: [0, 0, 0, 10] // Adjust the margin to position the text at the bottom
                                    });
                    ...
                    let mailOptions = {
                        ...
                        <body>
                            <h2>SALES REPORT</h2>
                            ${$('#transaction_list').html()} <!-- Include the HTML content of the transaction list -->
                            <p><strong>Total Sales:</strong> ${settings.symbol + parseFloat(sales).toFixed(2)}</p>
                        </body>
                        </html>
                        `
                    };
...
function loadSoldProducts() {
    ...
    sold.forEach((item, index) => {
        ...
        sold_list += `<tr>
            <td>${item.product}</td>
            <td>${item.qty}</td>
            <td>${product[0].stock == 1 ? product.length > 0 ? product[0].quantity : '' : ''}</td>
...
function userFilter(users) {
    ...
    users.forEach(user => {
        ...
        $('#users').append(`<option value="${user}">${u[0].username}</option>`);
    });
    ...
    if (allTransactions[index].paid != "") {
        payment = `<tr>
                ...
                <tr>
                    <td>Change</td>
                    <td>:</td>
                    <td>${settings.symbol + Math.abs(allTransactions[index].change_amount).toFixed(2)}</td>
                </tr>
                <tr>
                    <td>Method</td>
                    <td>:</td>
